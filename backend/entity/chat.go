...
type ChatSession struct {
	...
	UserID      int       `gorm:"column:user_id;not null;index:idx_chat_sessions_user_id" json:"userId"`                // 所属用户ID
	CharacterID int       `gorm:"column:character_id;not null;index:idx_chat_sessions_character_id" json:"characterId"` // 关联的角色卡ID
...
type Message struct {
	...
	SessionID        int       `gorm:"column:session_id;not null;index:idx_messages_session_id" json:"sessionId"`           // 所属会话ID
	...
	SortOrder        int       `gorm:"column:sort_order;not null;default:0;index:idx_messages_sort_order" json:"sortOrder"` // 排序顺序，决定消息在会话中的显示顺序
	CreatedAt        time.Time `gorm:"column:created_at;autoCreateTime" json:"createdAt"`                                   // 创建时间
	UpdatedAt        time.Time `gorm:"column:updated_at;autoUpdateTime" json:"updatedAt"`                                   // 更新时间
...
type MessageSwipe struct {
	...
	MessageID int       `gorm:"column:message_id;not null;index:idx_message_swipes_message_id" json:"messageId"` // 所属消息ID
