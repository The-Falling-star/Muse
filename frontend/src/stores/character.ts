...
export const useCharacterStore = defineStore('character', () => {
    ...
    const removeCharacter = (id: number) => {
        ...
        total.value -= 1;
        if (curCharId.value === id) {
            curCharId.value = null;
        }
        // 清除详情缓存
    ...
    // 选中角色
    const setCurCharId = (charId: number) => {
    ...
    // 获取角色详情
    const getCharDetail = async (id: number): Promise<Character | null> => {
        let char = characterDetailsCache.value.get(id);
        if (char) {
            return char;
        }
        const character = await characterClient.getCharacter({ id: id });
        char = character?.character;
        if (char) {
            characterDetailsCache.value.set(id, char);
            return char;
        }
        return null;
    }
